...
class HashSplitDict:
    ...
    def _load(self, path):
        ...
        for i in range(self.hlen):
            r[i] = ArrayDict(
                memmap_path=f"{path}/hid_%d.dict.arr" % i,
                memmap=True,
...
class ArrayDict:
    ...
    def __init__(
        ...
        if not is_New:
            self.HEAP = np.memmap(
                ...
                dtype=[("key", np.int64), ("value", value_dtype), ("next", np.int32)],
                mode=memmap_mode,
            )
            # print(self.HEAP)
        ...
        if is_New:  # 根据 item_size 新建  # second part of the critical problem
            if not memmap:
                self.HEAP = np.zeros(
                    (self.HASH_HEAP_SIZE + self.CHAIN_HEAP_SIZE),
                    dtype=[
                        ("key", np.int64),
                        ("value", self.value_dtype),
                        ("next", np.int32),
                    ],
                )
            else:
                self.HEAP = np.memmap(
                    ...
                    mode=memmap_mode,
                    shape=(self.HASH_HEAP_SIZE + self.CHAIN_HEAP_SIZE),
                )
    ...
    def __setitem__(self, key, value):
        ...
        while len(expect_cursor) != 0:
            ...
            self.set_value(
                expect_cursor[current_index], key[current_index], value[current_index]
            )
    ...
    def get_value(self, cursor, key):
        ...
        next_new_idx = np.where(
            (cursor_key != key) & (cursor_next == self.WITHOUT_NEXT_MAGIC)
        )[0]
        next_access_idx = np.where(
            (cursor_key != key)
        ...
        if len(next_access_idx) > 0:
            if self.item_get_cursor:
                ret[next_access_idx], ret_cursor[next_access_idx] = self.get_value(
                    cursor_next[next_access_idx], key[next_access_idx]
                )
            else:
                ret[next_access_idx] = self.get_value(
                    cursor_next[next_access_idx], key[next_access_idx]
                )
    ...
    def set_value(self, cursor, key, value):
        ...
        #
        if len(next_new_idx) > 0:
            new_idx = self.HEAP[self.CONFIG_CHAIN_CURSOR][0] + np.arange(
                len(next_new_idx)
            )
        ...
        if len(next_access_idx) > 0:
            self.set_value(
                cursor_next[next_access_idx],
                key[next_access_idx],
                value[next_access_idx],
            )
        return True
