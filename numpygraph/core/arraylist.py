...
class ArrayList:
    ...
    def flush(self):
        if len(self.arrays) != 0 and type(self.arrays[-1]) != np.memmap:
            ...
            array_memmap = np.memmap(
                self.path + ".%d" % (len(self.arrays) - 1),
                mode="w+",
                dtype=self.dtype,
    ...
    def close(self, merge=True):
        ...
        if merge is False:
            ...
            self.flush()
            opt = np.memmap(
                self.path, mode="w+", dtype=self.dtype, shape=(self.tail_cursor, 1)
            )
            cur = 0
            for array in self.arrays:
                opt[
                    cur : cur + min(array.shape[0], self.tail_cursor % self.chunk_size)
