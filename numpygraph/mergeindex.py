...
class MergeIndex:
    ...
    def merge_idx_to_callback(args):
        ...
        MergeIndex.toarrconcat[
            MergeIndex.edge_to_cursor : MergeIndex.edge_to_cursor + arr.shape[0]
        ]["value"] = arr["to"]
        MergeIndex.toarrconcat[
            MergeIndex.edge_to_cursor : MergeIndex.edge_to_cursor + arr.shape[0]
        ]["ts"] = arr["ts"]
    ...
    def merge_freq_idx_to(k, ipts):
        # 针对高频节点边表merge处理
        mems = [
            np.memmap(f, mode="r", dtype=[("to", np.int64), ("ts", np.int32)])
            for f in ipts
        ]
        return k, mems
        pass
    ...
    def merge_freq_idx_to_callback(args):
        ...
        for m in mems:
            MergeIndex.toarrconcat[
                MergeIndex.edge_to_cursor : MergeIndex.edge_to_cursor + m.shape[0]
            ]["value"] = m["to"]
            MergeIndex.toarrconcat[
                MergeIndex.edge_to_cursor : MergeIndex.edge_to_cursor + m.shape[0]
            ]["ts"] = m["ts"]
    ...
    def freq_idx_pointer_dump(graph):
        ...
        if len(MergeIndex.freq_idx_pointer) == 0:
            return
        idxarr = np.memmap(
            f"{graph}/edges_sort/hid_freq.idx.arr",
            mode="w+",
            order="F",
        ...
        for idx, (value, (index, length)) in enumerate(
            MergeIndex.freq_idx_pointer.items()
        ):
