...
class Read:
    ...
    def fetch_node_attr(self, _id):
        ...
        adict = ArrayDict(
            memmap_path=f"{self.graph}/nodes_mapper/{node_type}.dict.arr",
            value_dtype=[
                ("cursor", np.int64),
                ("chunk_id", np.int64),
    ...
    def fetch_edge_attr(self, fid, tid):
        ...
        # from/to short id
        adict = ArrayDict(
            memmap_path=f"{self.graph}/edges_mapper/hid_{fs_id}.dict.arr",
            value_dtype=[("index", np.int64), ("length", np.int32)],
            memmap_mode="r",
        )
        loc = adict[f_value_asarray]
        toarrconcat = np.memmap(
            f"{self.graph}/concat.to.arr",
            mode="r",
            order="F",
        ...
        for (index, length) in loc:
            for i in range(length):
                if (
                    toarrconcat[index + i][0] == fid
                    and toarrconcat[index + i][1] == tid
                ):
    ...
    def fetch_node_neibor_edges(self, _id):
        ...
        adict = ArrayDict(
            memmap_path=f"{self.graph}/edges_mapper/hid_{node_short_id}.dict.arr",
            value_dtype=[("index", np.int64), ("length", np.int32)],
            memmap_mode="r",
        )
        loc = adict[f_value_asarray]
